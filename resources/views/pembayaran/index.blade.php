                            ...
                            <div class="col-12">
                                <div class="card">
                                    ...
                                    <div class="card-content">
                                        <div class="card-body">
                                            @csrf
                                            <x-inputtext label="No. KK" value="{{ $pendaftar->no_pendaftaran }}" field="no_kk"
                                                icon="feather icon-credit-card" readonly="true" />
                                            <x-inputtext label="Nama Lengkap" value="{{ $pendaftar->nama_lengkap }}" field="nama_lengkap"
                                                icon="feather icon-user" readonly="true" />
                                            ...
                                            <x-inputtext label="Biaya Pendaftaran"
                                                value="{{ number_format($pendaftar->biaya_pendaftaran, '0', '', '.') }}" field="biaya_pendaftaran"
                                                icon="fa fa-money" readonly="true" />
                                            <div class="form-group">
                                                <input class="form-control" type="file" name="file" id="formFile">
                                            </div>
                                            <button type="submit" class="btn btn-danger btn-block mr-1 mb-1 btn-block">Konfirmasi
                                                Pembayaran</button>
                                        </div>
                                    </div>
                                </div>
                                            ...
                                            <tbody>
                                                @foreach ($pembayaran as $d)
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <a href="{{ asset('storage/uploads/' . $d->file) }}" target="_blank">Lihat Bukti</a>
                                                        </td>
                                                        <td>
                                                            @if ($d->status == '0')
                                                                <form method="POST" class="deleteform"
                                                                    action="/pembayaran/{{ Crypt::encrypt($d->no_transaksi) }}/delete">
                                                                    @csrf
                                                                    @method('DELETE')
